...
def center_window(self, w, h): # Pass in the tkinter frame (master) reference and the w and h
    ...
    # Calcutlate x and y coordinated to paint the app centered on the user's screen
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # Enforce the user to provide both names
        ...
        with conn:
            cursor = conn.cursor()
            # Check the databasse for existance of the fullname, if so we will alert the user and disregard request
...
def onDelete(self):
    ...
    with conn:
        ...
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel("Dlete Confirmation","All information associated with, ({}) \nwill be permanently deleted from the database. \n\nProceed with the delete request?".format(var_select))
            if confirm:
                conn = sqlite3.connect('db_phonebook.db')
                with conn:
                    cursor=conn.cursor()
...
def onUpdate(self):
    try:
        ...
        var_value = self.lstList1.get(var_select) # list sleection's text value
    except:
